...
const { inferFunctionDescription, inferParamDescription, inferClassDescription } = require("./inferrer.js");
...
function extractThrowsFromBody(body) {
    if (!body || body.kind !== ts.SyntaxKind.Block) return [];
    var seen = new Set();
    var result = [];
    function walk(n) {
        if (ts.isThrowStatement(n)) {
            ...
            var typeName = "Error";
            if (expr && ts.isNewExpression(expr) && expr.expression) typeName = expr.expression.getText();
            else if (expr && ts.isIdentifier(expr)) typeName = expr.getText();
            if (!seen.has(typeName)) { seen.add(typeName); result.push({ type: typeName }); }
            return;
        }
...
function buildParamLines(params) {
    return (params || []).map((p) => {
        ...
        const desc = inferParamDescription(name);
        return ` * @param {${type}} ${label} - ${desc}.`;
    });
}

function buildFunctionDoc({ name, params, returnType, mods, isGenerator, throws }) {
    const desc = inferFunctionDescription(name, mods);
    const lines = ["/**", ` * ${desc}`];
    ...
    if (throws && throws.length) {
        throws.forEach(function(t) { lines.push(` * @throws {${t.type}}`); });
    }
    lines.push(" */");
    return lines;
}
...
function buildClassDoc(node) {
    ...
    const desc = inferClassDescription(name);
    const lines = ["/**", ` * ${desc}`];
...
function buildMethodDoc(node) {
    ...
    const desc = isGetter
        ? "Returns the " + inferParamDescription(name) + "."
        : isSetter
            ? "Sets the " + inferParamDescription(name) + "."
            : inferFunctionDescription(name, mods);
    const lines = ["/**", ` * ${desc}`];
    ...
    const bodyThrows = extractThrowsFromBody(node.body);
    if (bodyThrows.length) bodyThrows.forEach(function(t) { lines.push(` * @throws {${t.type}}`); });
    lines.push(" */");
    return lines;
}
...
function buildSingleVariableDoc(decl, isConst) {
    ...
    if (isFunctionLikeInitializer(init)) {
        return buildFunctionDoc({
            ...
            throws: extractThrowsFromBody(init.body),
        });
    }
        ...
        if (ts.isFunctionDeclaration(node) && node.body) {
            addEdit(node, () =>
                buildFunctionDoc({
                    ...
                    throws: extractThrowsFromBody(node.body),
                }),
            );
...
function processFile(filePath, options = {}) {
    const { write = false, force = false, silent = false, dryRun = false } = options;
    ...
    if (!silent) {
        ...
        console.log(`  ${filePath} (${edits.length} block${edits.length === 1 ? "" : "s"} ${verb})`);
    }
    return edits.length;
}
 ...
 * @returns {{ documented: number, total: number, undocumented: number }}
 */
function analyseFile(filePath) {
    const sourceText = fs.readFileSync(filePath, "utf8");
    const sourceFile = ts.createSourceFile(filePath, sourceText, ts.ScriptTarget.Latest, true, getScriptKind(filePath));
    const undocumented = collectEdits(sourceFile, false).length;
    const total = collectEdits(sourceFile, true).length;
    return { documented: total - undocumented, total, undocumented };
}
