...
const { lintModule, KNOWN_TAGS, NO_DESC_TAGS, tagTrailingText } = require("./lint.js");
...
function lightFixGenericBlock(sym, rules, indent) {
    ...
    if (rules.has("empty-tags")) {
        lines = lines.map((l) => {
            ...
            if (m && NO_DESC_TAGS.has(m[1].toLowerCase())) {
                ...
                const trailing = tagTrailingText(m);
                if (trailing) {
                    changed = true;
    ...
    if ((rules.has("no-blank-block-descriptions") || rules.has("require-description")) && !sym.description) {
        if (lines.length === 1) {
            // 2026-07-31 fix: a single-line raw comment (e.g. "/** */") has no
            // index-1 line to splice before -- `lines.splice(1, 0, ...)` on a
            // 1-element array inserts AFTER the only element, landing the new
            // text outside the closing "*/" and corrupting the source file
            // (reproduced live: `/** */\nclass Widget {}` --fix --write produced
            // a SyntaxError). Expand into a proper multi-line block instead,
            // using the same per-continuation-line indent convention
            // rebuildFunctionBlock() uses. Any real inner content between
            // "/**" and "*/" (there normally isn't any -- sym.description
            // would be truthy otherwise -- but a tag-only single-liner like
            // "/** @internal */" is possible) is preserved as its own line
            // rather than silently dropped.
            var m = lines[0].match(/^\s*\/\*\*([\s\S]*?)\*\/\s*$/);
            if (m) {
                var inner = m[1].trim();
                lines = ["/**"]
                    .concat(inner ? [indent + " * " + inner] : [])
                    .concat([indent + " * " + TODO_DESCRIPTION, indent + " */"]);
                changed = true;
            }
            // If the single line doesn't match the expected "/** ... */"
            // shape at all, leave it untouched rather than guess -- no-op is
            // always safer than a corrupting rewrite.
        } else {
...
function fixModule(filePath) {
    ...
    function tryGeneric(sym, symbolKey) {
        ...
        const indent = indentOf(sourceText, sym.commentRange.pos);
        const newText = lightFixGenericBlock(sym, rules, indent);
