 ...
 * @param {RegExpMatchArray} tagMatch - result of stripped.match(/^@([a-zA-Z][\w-]*)\s*(.*)$/)
 * @returns {string}
 */
function tagTrailingText(tagMatch) {
    return tagMatch[2].replace(/\*\/\s*$/, "").trim();
}
...
function lintEmptyTags(issues, symbol, rawComment, line) {
    ...
    rawComment.split("\n").forEach(function (l) {
        ...
        if (m && NO_DESC_TAGS.has(m[1].toLowerCase()) && tagTrailingText(m)) {
...
// tagTrailingText is an internal sharing detail (lib/fix.js reuses it so the
// empty-tags rule's "does this tag have trailing text" logic can't drift
// between lint and fix) rather than a designed public-API addition -- kept
// out of lint.d.ts's hand-written public declarations for that reason, same
// as this file's other unexported helpers (push, lintTagNames, etc.).
module.exports = { lintModule, KNOWN_TAGS, NO_DESC_TAGS, tagTrailingText };
