...
const { findFirstDescendant, asFunctionLike, asClass } = require("./ast-utils.js");
...
function emptyJSDoc() {
    return { description: null, example: null, since: null, deprecated: null, params: [], returns: null, throws: [], rawComment: null, badComment: null, commentRange: null };
}
 ...
 */
function looksLikeIntendedJSDoc(text) {
    if (!text.startsWith("/*") || text.startsWith("/**")) return false;
    const opensBad = /^\/\*{3,}/.test(text) || /^\/\*[^*]/.test(text);
    return opensBad && /@[a-zA-Z]/.test(text);
}
...
function readJSDoc(sourceFile, node) {
    ...
    for (let i = ranges.length - 1; i >= 0; i--) {
        ...
        if (text.startsWith("/**")) {
            const parsed = parseJSDocBlock(text);
            parsed.rawComment = text;
            parsed.badComment = null;
            // Exact source range of this block — additive, used only by lib/fix.js's
            // --lint --fix to splice a replacement block in at the right offset.
            // Never read by any --check/--check-drift/--lint code path.
            parsed.commentRange = { pos: r.pos, end: r.end };
            return parsed;
        }
        if (badComment === null && looksLikeIntendedJSDoc(text)) badComment = text;
    }
    const empty = emptyJSDoc();
    empty.badComment = badComment;
    return empty;
}
...
function extractFunction(node, sourceFile, nameOverride) {
    ...
    return {
        ...
        throws: jsdoc.throws,
        rawComment: jsdoc.rawComment,
        badComment: jsdoc.badComment,
                commentRange: jsdoc.commentRange,
    ...
    for (const m of node.members) {
        ...
        if (ts.isConstructorDeclaration(m)) {
            result.constructor = {
                ...
                rawComment: mjsdoc.rawComment,
                badComment: mjsdoc.badComment,
                commentRange: mjsdoc.commentRange,
            };
            ...
            const mm = modifiersOf(m);
            result.getters.push({ name: m.name.getText(), returnType: inferReturnType(m), isStatic: mm.isStatic, description: mjsdoc.description, deprecated: mjsdoc.deprecated, since: mjsdoc.since, rawComment: mjsdoc.rawComment, badComment: mjsdoc.badComment, commentRange: mjsdoc.commentRange });
        } else if (ts.isSetAccessorDeclaration(m)) {
            const mm = modifiersOf(m);
            result.setters.push({ name: m.name.getText(), params: extractParams(m.parameters), isStatic: mm.isStatic, description: mjsdoc.description, deprecated: mjsdoc.deprecated, rawComment: mjsdoc.rawComment, badComment: mjsdoc.badComment, commentRange: mjsdoc.commentRange });
            ...
            for (const decl of node.declarationList.declarations) {
                ...
                if (asFunctionLike(init)) {
                    result.functions.push({
                        ...
                        rawComment: stmtJsdoc.rawComment,
                        badComment: stmtJsdoc.badComment,
                commentRange: stmtJsdoc.commentRange,
