...
// "bench" added 2026-07-31: bench/generated/ is a gitignored synthetic
// perf-fixture tree (see bench:perf-gate) whose fixture dirs are named
// models/controllers/services/routes/etc. -- exactly the directory names
// getArchitecturePatterns() below matches on, so an un-ignored bench/
// left on disk after a local perf-gate run silently false-positives
// patterns (MVC, Repository Pattern, ...) for the whole project.
// "docs-dashboard" and "_site" added same day, same root cause: both are
// this repo's own gen-docs *output* directories (`npm run dashboard`,
// `npm run docs:pages`), fully gitignored (.gitignore /docs-dashboard/,
// _site/) yet left on disk between local runs. gen-docs's own generated
// site always has a modules/ folder, which false-positived "Feature-Based"
// architecture (findTopNode("modules") matched docs-dashboard/modules/,
// not any real source directory). Deliberately NOT adding "docs" here --
// unlike the other two it's gitignored but also intentionally shown via
// KNOWN_DIR_DESCRIPTIONS (it can legitimately hold docs/backlog content
// too), so blanket-ignoring it would drop a real signal, not just noise.
const IGNORE_DIR_NAMES = new Set(["node_modules", ".git", "coverage", "dist", "build", "out", ".cache", ".turbo", ".next", "bench", "docs-dashboard", "_site"]);
 ...
 * @param {string} rootDir
 * @param {object[]} [precomputedStructure] - a `getProjectStructure()` result to
 ...
 */
function getFrameworkSignals(rootDir, precomputedStructure) {
    const signals = [];
    ...
    const structure = precomputedStructure || getProjectStructure(rootDir);
 ...
 * @param {string} rootDir
 * @param {object[]} [precomputedStructure] - see getFrameworkSignals()'s
 ...
 */
function getArchitectureSignals(rootDir, precomputedStructure) {
    const signals = [];
    const pkg = readJsonSafe(path.join(rootDir, "package.json"), {});
    ...
    const dependencyFrameworks = getFrameworkSignals(rootDir, precomputedStructure).filter(function (s) { return s.confidence === "dependency"; });
    ...
    const dirNames = collectDirectoryNamesFromStructure(precomputedStructure || getProjectStructure(rootDir));
...
function getAllFacts(rootDir) {
    ...
    // own default of 3) and reused for the `structure` field AND every
    // category below that needs a directory tree -- getFrameworkSignals(),
    // getArchitectureSignals() (which internally reuses the same structure
    // for its own getFrameworkSignals() call too), and getArchitecturePatterns().
    // 2026-07-31: previously only architecturePatterns reused this -- the
    // other two each did their own separate (up to 3 combined) redundant
    // walks every getAllFacts() call. See getArchitecturePatterns()'s own
    // doc comment for why depth is capped at 4, not deeper.
    ...
    return {
        ...
        frameworkSignals: getFrameworkSignals(dir, structure),
        architectureSignals: getArchitectureSignals(dir, structure),
