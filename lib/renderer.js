...
(function(){
  ...
  // Search
  var INDEX=(window.__SEARCH_INDEX__||[]).map(function(r){return{name:r.name,kind:r.kind,module:r.module,body:r.body,url:_pfx+r.url,nameLower:(r.name||'').toLowerCase(),moduleLower:(r.module||'').toLowerCase(),bodyLower:r.body?r.body.toLowerCase():''};});
  ...
  if(box&&panel){
    ...
    function search(q){
      ...
      render(INDEX.filter(function(r){return r.nameLower.includes(q)||r.moduleLower.includes(q)||r.bodyLower.includes(q);}));
    }
...
function buildQualitySection(quality) {
    ...
    if (graph) {
        var inDegreeEntries = Object.keys(graph.inDegree || {})
            .map(function (f) { return [toDisplayPath(f), graph.inDegree[f]]; })
        ...
        orphansCard = qCard({
            title: "Orphan files",
            statLineHtml: "",
            rows: orphans.map(function (f) { return [toDisplayPath(f), null, null]; }),
 ...
 */
function buildHealthDetailPages(quality, modules, projectName, version, tree, switcherHtml, defaultCache, parentsLinked, architectureHref, seoBaseUrl, seoDescription) {
    var result = quality.result;
    var graph = quality.graph;
    var orphans = quality.orphans || [];
    var pages = [];
    ...
    function wrap(slug, title, tableHtml) {
        ...
        var seo = seoBaseUrl ? { description: seoDescription, canonical: seoBaseUrl + slug } : (seoDescription ? { description: seoDescription } : null);
        pages.push({ path: slug, html: page(title + " - " + projectName, sidebar, body, "", topnav, null, seo) });
    }
    ...
    wrap(
        ...
        '<div class="qtable-wrap"><table class="qtable"><thead><tr><th>File</th></tr></thead><tbody>' +
            orphanRowsHtml(orphans.map(toDisplayPath)) +
            "</tbody></table></div>",
    );
 ...
 * @param {string} [seoBaseUrl] - trailing/optional, mirrors buildHealthDetailPages()'s
 ...
 * @param {string} [seoDescription] - meta description; falls back to
 ...
 */
function buildArchitecturePage(facts, modules, projectName, version, tree, switcherHtml, defaultCache, parentsLinked, seoBaseUrl, seoDescription) {
    ...
    var seo = { description: seoDescription || ARCHITECTURE_ORIENTATION_PARAGRAPH };
    if (seoBaseUrl) seo.canonical = seoBaseUrl + "architecture.html";
    return { path: "architecture.html", html: page("Architecture - " + projectName, sidebar, body, "", topnav, null, seo) };
}
 ...
 */
// 2026-07-31 perf fix: buildFunctionHealthLookup() is called once PER MODULE
// from buildSite()'s modules.forEach (same `options.quality` object every
// call), and previously did a fresh linear .find() over quality.result.files
// on every single call -- O(modules x files). Same class of bug the v2.4.2
// linear-scaling fix (see commonRootCache above / adr-linear-scaling-fix.md)
// already closed elsewhere in this file, just not caught here. Fixed the
// same way: a WeakMap keyed on `quality.result` (not `quality` itself, since
// callers construct a fresh `{ result: {...} }` wrapper in some places --
// keying on the stable inner object is what actually gives cross-call
// cache-hits within one buildSite() run) holding a resolved-path -> file
// Map, built once and reused for every module.
var fileReportMapCache = new WeakMap();
function getFileReportMap(qualityResult) {
    if (fileReportMapCache.has(qualityResult)) return fileReportMapCache.get(qualityResult);
    var map = new Map();
    (qualityResult.files || []).forEach(function (f) {
        map.set(path.resolve(f.filePath), f);
    });
    fileReportMapCache.set(qualityResult, map);
    return map;
}
...
function buildFunctionHealthLookup(quality, filePath) {
    ...
    var fileReport = getFileReportMap(quality.result).get(key);
 ...
 * callers opt in.
 * @param {{description?: string, canonical?: string}} [seo]
 */
function page(title, sidebarHtml, bodyHtml, assetPrefix, topnavHtml, tocHtml, seo) {
    var p = assetPrefix || '';
    var hasToc = !!tocHtml;
    var tocCol = hasToc ? '<aside class="toc" id="toc">' + tocHtml + '</aside>' : '';
    var s = seo || {};
    var head = '<meta charset="UTF-8">\n<meta name="viewport" content="width=device-width,initial-scale=1">\n<title>' + esc(title) + '</title>\n';
    if (s.description) head += '<meta name="description" content="' + esc(s.description) + '">\n';
    head += '<meta name="robots" content="index, follow">\n';
    if (s.canonical) {
        head += '<link rel="canonical" href="' + esc(s.canonical) + '">\n';
        head += '<meta property="og:type" content="website">\n';
        head += '<meta property="og:title" content="' + esc(title) + '">\n';
        if (s.description) head += '<meta property="og:description" content="' + esc(s.description) + '">\n';
        head += '<meta property="og:url" content="' + esc(s.canonical) + '">\n';
    }
    return '<!DOCTYPE html>\n<html lang="en">\n<head>\n' + head + '<link rel="stylesheet" href="' + p + 'assets/style.css">\n</head>\n<body>\n'
        + (topnavHtml || '')
...
function buildSite(modules, options) {
    ...
    // seo-organic-growth-foundation: optional, additive SEO metadata for every
    // generated page. baseUrl (e.g. "https://example.com/docs/") turns on
    // canonical links + OpenGraph tags site-wide; description is the meta
    // description fallback for pages with no more specific text of their own
    // (module pages prefer mod.description when present -- see below). Neither
    // option existing before this addition, and omitting both reproduces the
    // exact previous output.
    var seoBaseUrl = options.baseUrl || null;
    var seoDescription = options.description || null;
    ...
    var archPage = (options.facts && !isSnapshot)
        ? buildArchitecturePage(options.facts, modules, projectName, version, tree, switcherRoot, defaultCacheRoot, true, seoBaseUrl, seoDescription)
        : null;
    ...
    var idxSeo = { description: seoDescription };
    if (seoBaseUrl) idxSeo.canonical = seoBaseUrl + 'index.html';
    pages.push({ path: 'index.html', html: page(projectName, sidebarIdx, idxBody, '', topnavIdx, null, idxSeo) });

    if (archPage) pages.push(archPage);

    var fileHealthLookup = options.quality ? buildFileHealthLookup(options.quality) : {};
    if (options.quality) {
        pages = pages.concat(buildHealthDetailPages(options.quality, modules, projectName, version, tree, switcherRoot, defaultCacheRoot, true, architectureHrefIdx, seoBaseUrl, seoDescription));
    }
    ...
    modules.forEach(function (mod) {
        ...
        // Prefer this module's own top-of-file JSDoc description (real,
        // per-page, already-available text) over the site-wide fallback --
        // every module page gets a distinct meta description instead of
        // 30+ pages sharing identical copy, when the source file has one.
        var modSeo = { description: mod.description || seoDescription };
        if (seoBaseUrl) modSeo.canonical = seoBaseUrl + rel;
        pages.push({
            path: rel,
            html: page(
                label + ' - ' + projectName,
